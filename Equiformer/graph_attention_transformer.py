...
from Equiformer.radial_basis import RadialBasis
...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        drop_path_rate=0.0,
        mean=None, std=None, scale=None, atomref=None, out_channels=1):
        super().__init__()
        ...
        self.head = torch.nn.Sequential(
            ...
            LinearRS(self.irreps_feature, o3.Irreps(f'{self.out_channels}x0e'), rescale=_RESCALE)) 
    ...
    def forward(self, f_in, pos, batch, node_atom, **kwargs) -> torch.Tensor:
        ...
        node_atom = node_atom.new_tensor(lookup_table)[node_atom]
...
def graph_attention_transformer_l2(irreps_in, radius, num_basis=128, 
    atomref=None, task_mean=None, task_std=None, out_channels=1, **kwargs):
    model = GraphAttentionTransformer(
        ...
        mean=task_mean, std=task_std, scale=None, atomref=atomref, out_channels=out_channels)
    return model
