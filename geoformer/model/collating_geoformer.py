...
class GeoformerDataCollator:
    ...
    def __call__(self, features: List[dict]) -> Dict[str, Any]:
        ...
        batch["spec_x"] = torch.concatenate([feat["spec_x"] for feat in features])
        batch["spec_y"] = torch.concatenate([feat["spec_y"] for feat in features])

        batch["name"] = [f.get("name") for f in features]
